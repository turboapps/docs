...
- **Import Configuration** button allows an application package to be imported from a file (.ini, .msi, .axt, .appv), the turbo.net hub, or the local image repository. For more information, see [Import Configuration](../../studio/working-with-turbo-studio/import-configuration.html).
