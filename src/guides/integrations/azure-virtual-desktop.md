...
- **Turbo Client Installer**: The installer for Turbo Client. You can download it from [Turbo.net](https://turbo.net/download)
...
2. Authenticate with your Turbo Portal using an [API Key](/server/administration/users.html#api-keys).
   ...
   ```batch
   # Login using API key
   turbo login --all-users --api-key=key-here
   ```
...
1. When preparing your AVD gold image, install and configure Turbo Client.
   ...
   # Note: Open a new command prompt after installation to get updated PATH
   turbo config --all-users --image-path=allusers

   # Set the Turbo Server URL
   turbo config --all-users --domain=https://turbo.mycompany.com
   
   # Enable auto registration of applications on user login
   turbo config --all-users --enable=AutoRegister
   ```
...
2. Authenticate to your Turbo Server and subscribe to your workspace.
   ...
   turbo subscription --all-users --all resume
   ...
   turbo subscription --all suspend --all-users
...
1. Access the [AVD Portal](https://aka.ms/avdweb)
